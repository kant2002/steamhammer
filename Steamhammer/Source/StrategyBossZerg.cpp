...
void StrategyBossZerg::makeUrgentReaction(BuildOrderQueue & queue)
{
        ...
        for (Base * base : the.bases.getAll())
        {
            if (base->getOwner() == the.self() && base->isCompleted())
            {
                BWAPI::Unitset bunkers =
                    ...
                    BWAPI::Filter::GetType == BWAPI::UnitTypes::Terran_Bunker && BWAPI::Filter::GetPlayer == the.enemy());
        ...
        for (Base * base : the.bases.getAll())
        {
            if (base->getOwner() == the.self() && base->isCompleted())
            {
                BWAPI::Unit cannon =
                    ...
                    BWAPI::Filter::GetType == BWAPI::UnitTypes::Protoss_Photon_Cannon && BWAPI::Filter::GetPlayer == the.enemy(),
                    14 * 32);
                if (cannon)
                {
		...
		(!outOfBook || queue.size() < 5))		// don't loop beyond this
	{
		...
		int nScourgeNeeded = std::min(12, the.info.nScourgeNeeded());
		int nToMake = 0;
...
void StrategyBossZerg::checkGroundDefenses(BuildOrderQueue & queue)
{
	...
	if (OpponentModel::Instance().getEnemyPlan() == OpeningPlan::HeavyRush &&
		...
		totalSunkens == 0 &&
		the.bases.baseCount(_self) > 1)		// if we're playing a 1-base strategy, we may be OK
	{
	...
	if (nDrones >= 9 && totalSunkens < 5)
	{
		...
		{
			if (nDrones < 12 ||
				nDrones < 20 && _enemyRace != BWAPI::Races::Zerg)
			{
    ...
    for (Base * base : the.bases.getAll())
    {
            ...
            (expectDrop || !base->getNatural() || base->getNatural()->getOwner() != the.self()))
        {
            ...
            if (sunks.empty())
            {
                ...
                if (base->getMain())
                {
                    // This base is the natural of some main base. Place the sunk forward as usual.
                    where = base->getFrontTile();
                }
...
// Used for the spawning pool and the spire, and perhaps others.
...
// Set flags or counts to recommend that certain abilities are worth having.
void StrategyBossZerg::recommendTech()
{
    if (_enemyRace == BWAPI::Races::Terran)
    {
        ...
        _recommendEnsnare =
            ...
            4 * the.your.seen.count(BWAPI::UnitTypes::Terran_Wraith) +
    ...
    else if (_enemyRace == BWAPI::Races::Protoss)
    {
        ...
        _recommendEnsnare =
            ...
            the.your.seen.count(BWAPI::UnitTypes::Protoss_Dark_Templar) >= 8)
                ? 1 : 0;
        _recommendBroodling =
    ...
    {
        _recommendEnsnare = std::min(_recommendEnsnare, 1);
    }
		...
		if (!type.isWorker() && !type.isBuilding() && type != BWAPI::UnitTypes::Protoss_Interceptor)
		{
			...
			if (type.isFlyer())
			{
				...
				if (type == BWAPI::UnitTypes::Protoss_Corsair || type == BWAPI::UnitTypes::Protoss_Scout)
				{
					...
					techScores[int(TechUnit::Guardians)] -= 2 * count * type.supplyRequired();
					techScores[int(TechUnit::Devourers)] += 2 * count * (type.supplyRequired() + 4);
				}
				else if (type == BWAPI::UnitTypes::Protoss_Carrier)
				{
					techScores[int(TechUnit::Guardians)] -= 2 * count * (type.supplyRequired() + 2);
...
void StrategyBossZerg::vTerranTechScores(const PlayerSnapshot & snap)
{
    ...
    for (const std::pair<BWAPI::UnitType, int> & unitCount : snap.getCounts())
	{
        ...
        {
            techScores[int(TechUnit::Hydralisks)] += count * 6;
            techScores[int(TechUnit::Mutalisks)] -= count * 2;
            techScores[int(TechUnit::Guardians)] -= count * 4;
            techScores[int(TechUnit::Devourers)] += count * 6;
...
void StrategyBossZerg::chooseUnitMix()
{
	...
	else if (bestUnit == TechUnit::Devourers && nDevourers < devourerLimit())
	{
...
void StrategyBossZerg::produceOtherStuff(int & mineralsLeft, int & gasLeft, bool hasEnoughUnits)
{
    ...
    if (nEvo > 0 && nDrones >= 12 && nGas > 0 &&
        hasPool && nFreeEvo() > 0 &&
        !_emergencyGroundDefense && hasEnoughUnits &&
        (!the.bases.isIslandStart() || hasHiveTech))
    {
        if (_enemyRace == BWAPI::Races::Zerg)
        {
            if (attackUps == 0 ||
